...
st.write(f'ようこそ ゲストさん')
st.title('Airwork採用管理 応募解析')
df = st.file_uploader("ファイルアップロード", type='csv')

if df:
    st.sidebar.write('応募者分析したい日時を指定してください。<span style="color:red;background:pink">※無料デモは直近６日しか選択できません。有料版は直近２年間選択できます。</span>',unsafe_allow_html=True)
